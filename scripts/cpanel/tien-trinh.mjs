// Xem / tắt tiến trình Node của bộ não trên hosting — cho tài khoản cPanel KHÔNG có shell.
// Setup Node.js App -> Run JS script:
//   tien-trinh:xem   liệt kê tiến trình của tài khoản (pid, giờ bật, thư mục, lệnh), đuôi stderr.log,
//                    và packages/xeon/dist/main.js là bản build thật hay tệp mẫu "It works!"
//   tien-trinh:tat   tắt tiến trình web (lsnode/Passenger) của CHÍNH app này; lượt truy cập sau LiteSpeed
//                    bật tiến trình mới nạp bản vừa build. Không đụng app khác (vd landing), không đụng lệnh này.
// Cùng công cụ với server-khach (15/09/2026): build xong, bấm Restart mà LiteSpeed vẫn giữ tiến trình cũ.
...
const mainFile = path.join(root, "packages", "xeon", "dist", "main.js");
if (!fs.existsSync(mainFile)) console.log("[tien-trinh] packages/xeon/dist/main.js: KHÔNG CÓ — chưa build");
else {
  const text = fs.readFileSync(mainFile, "utf8");
  const kind = text.includes("It works!") ? "TỆP MẪU 'It works!' (chưa build đè)" : text.includes("buildXeonApp") ? "bản build thật" : "không nhận ra";
  console.log(`[tien-trinh] packages/xeon/dist/main.js: ${kind}, sửa lúc ${time(fs.statSync(mainFile).mtime)}`);
}
...
console.log("[tien-trinh] Xong. Mở lại trang: LiteSpeed sẽ bật tiến trình mới với bản vừa build.");
