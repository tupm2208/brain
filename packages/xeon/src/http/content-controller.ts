...
const PATHS_OWNED = new Set<string>([PATHS.contentReview, PATHS.contentOptimize, PATHS.contentTrends]);
...
function post(v: unknown): ReviewPost {
  const o = asObject(v);
  return {
    ma: text(o["ma"], 120), gio: text(o["gio"], 10), trang: text(o["trang"], 120), dangBai: text(o["dangBai"], 60), huongDan: text(o["huongDan"], 800),
  ...
  async handle(req: IncomingMessage, res: ServerResponse, ctx: RequestContext): Promise<boolean> {
    ...
    try {
      if (ctx.path === PATHS.contentReview) {
        return this.reply(res, await this.options.desk.review({ tenant: shop, bai: post(body["bai"]), loiLuat: strings(body["loiLuat"], 30, 300), phongCach: style(body["phongCach"]), kienThuc: text(body["kienThuc"], 6000) }));
      }
