...
exports.OPTIONAL_TEMPLATES = exports.ENGINE_VARS = void 0;
exports.validatePack = validatePack;
exports.assertPackValid = assertPackValid;
exports.kiemTuDemVoiMucLuc = kiemTuDemVoiMucLuc;
const contract_1 = require("@sp/contract");
const turn_1 = require("../engine/turn");
const text_1 = require("../engine/text");
...
function hasDiacritics(v) {
    ...
    return (0, text_1.stripDiacritics)(v) !== v.toLowerCase();
}
function validatePack(pack) {
    const problems = [];
    const p = (m) => { problems.push(m); };
    if ((0, text_1.normalize)(pack.id) === "")
        p("Bo luat thieu ma dinh danh (`id`).");
    if (pack.identity.customerPronoun.trim() === "")
        p("Thieu cach xung ho voi khach.");
    if (pack.identity.selfPronoun.trim() === "")
        p("Thieu cach shop tu xung.");
    for (const t of pack.allowedTools) {
        if (!(0, contract_1.isToolName)(t))
            p(`Cong cu khong ton tai trong ban giao keo: "${String(t)}".`);
    }
    // --- truc bien the ---
    const axes = pack.itemShape.axes;
    if (axes.length === 0)
        p("Hinh dang mon hang phai co it nhat mot truc bien the.");
    const axisIds = new Set();
    for (const axis of axes) {
        if (axisIds.has(axis.id))
            p(`Truc bien the trung ma: "${axis.id}".`);
        axisIds.add(axis.id);
        if (!/^[a-z][a-z0-9_]*$/.test(axis.id)) {
            p(`Ma truc "${axis.id}" phai viet thuong khong dau, khong khoang trang.`);
        }
        try {
            new RegExp(axis.pattern);
        }
        catch {
            p(`Truc "${axis.id}" co mau bat hong: ${axis.pattern}`);
        }
        for (const c of axis.canonical) {
            try {
                ...
                p(`Truc "${axis.id}" co mau quy chuan hong: ${c.pattern}`);
            }
        }
        if (axis.label.trim() === "")
            p(`Truc "${axis.id}" thieu nhan hien cho nguoi doc.`);
        if (axis.examples.length === 0)
            p(`Truc "${axis.id}" phai co it nhat mot vi du de tu kiem.`);
        for (const ex of axis.examples) {
            const got = (0, turn_1.extractAxis)(axis, ex.text);
            if (got !== ex.expect) {
        ...
        for (const ex of axis.examples) {
            if (ex.expect === null)
                continue;
            const once = (0, turn_1.canonicalValue)(axis, ex.expect);
            if ((0, turn_1.canonicalValue)(axis, once) !== once) {
                p(`Truc "${axis.id}": quy chuan khong on dinh voi "${ex.expect}" (chay hai lan ra khac nhau).`);
            }
        }
    }
    // --- mau cau ---
    const known = new Set([...exports.ENGINE_VARS, ...Object.keys(pack.extraValues ?? {})]);
    ...
    if (usesStock)
        required.push("in_stock", "out_of_stock");
    if (pack.lexicon.knownBrandsNotCarried.length > 0)
        required.push("brand_not_carried");
    ...
    const moiMauCau = Object.entries(pack.templates)
    ...
    for (const { key, text } of moiMauCau) {
        ...
        for (const phrase of pack.identity.neverSay) {
            if ((0, text_1.stripDiacritics)(text).includes((0, text_1.stripDiacritics)(phrase))) {
    ...
    if (pack.intents.length === 0)
        p("Bo luat khong khai y dinh nao.");
    ...
    const seen = new Set();
    const validSlots = new Set(["item", "phone", "topic", ...axisIds]);
    for (const intent of pack.intents) {
        if (seen.has(intent.id))
            p(`Y dinh trung ma: "${intent.id}".`);
        seen.add(intent.id);
        ...
        for (const t of intent.tools) {
            if (!pack.allowedTools.includes(t)) {
            ...
            else if (!turn_1.DISPATCHABLE_TOOLS.includes(t)) {
                ...
                p(`Y dinh "${intent.id}" goi cong cu "${t}" nhung bo may chua co cach chay no. ` +
                    `Cong cu chay duoc: ${turn_1.DISPATCHABLE_TOOLS.join(", ")}.`);
            }
        }
        for (const rx of intent.patterns ?? []) {
            try {
                ...
                p(`Y dinh "${intent.id}" co mau nhan dang hong: ${rx}`);
            }
        }
        if (intent.template.trim() === "")
            p(`Y dinh "${intent.id}" thieu mau cau tra loi.`);
        if (intent.askBackTemplate.trim() === "")
            p(`Y dinh "${intent.id}" thieu mau cau hoi lai.`);
    ...
    for (const intent of pack.intents) {
        for (const ex of intent.examples ?? []) {
            const got = (0, turn_1.detectIntent)(pack, ex);
            if (got?.id !== intent.id) {
    ...
    for (const must of ["no_unsourced_numbers", "forbidden_phrases", "no_facts_when_offline"]) {
        if (!kinds.has(must))
            p(`Bo luat thieu cong bat buoc "${must}".`);
    }
    ...
    for (const phrase of pack.identity.neverSay) {
        if (!hasDiacritics(phrase))
            p(`Cum cam "${phrase}" phai viet co dau day du.`);
    }
    for (const g of pack.gates) {
        ...
        if (g.kind === "forbidden_patterns") {
            if (g.patterns.length === 0)
                p("Cong `forbidden_patterns` khong co mau nao.");
            for (const rx of g.patterns) {
                try {
                    new RegExp(rx, "u");
                }
                catch {
                    p(`Cong \`forbidden_patterns\` co mau hong: ${rx}`);
                }
                if (!hasDiacritics(rx))
                    p(`Mau cam "${rx}" phai viet co dau — cong soi cau bot tren dang co dau.`);
            }
            if (g.reason.trim() === "")
                p("Cong `forbidden_patterns` phai noi ro ly do de nguoi van hanh doc nhat ky hieu duoc.");
        }
        if (g.kind === "require_source_for_claims") {
            for (const t of g.topics) {
                if (!hasDiacritics(t))
                    p(`Chu de chinh sach "${t}" phai viet co dau day du.`);
            }
            for (const rx of g.patterns ?? []) {
                try {
                    new RegExp(rx, "u");
                    ...
                    p(`Cong \`require_source_for_claims\` co mau hong: ${rx}`);
                }
                if (!hasDiacritics(rx))
                    p(`Mau chinh sach "${rx}" phai viet co dau.`);
            }
        }
    }
    return problems;
}
function assertPackValid(pack) {
 ...
 */
function kiemTuDemVoiMucLuc(pack, items) {
    const dem = new Set((pack.lexicon.fillerWords ?? []).map(text_1.normalize));
    const pham = new Set();
    for (const it of items) {
        ...
        for (const t of (0, text_1.tokens)(`${it.code} ${it.name}`))
            if (t.length >= 3 && dem.has(t))
                pham.add(t);
    }
    return [...pham].sort();
}
