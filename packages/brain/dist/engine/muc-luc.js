    ...
    return {
        nap(tenant, pack, items) {
            ...
            for (const it of items) {
                if (it.tenant !== tenant) {
                    throw new Error(`Muc luc cua shop "${tenant}" mang mon "${it.code}" cua shop "${it.tenant}" — tu choi ca dot nap.`);
                }
                if (daThay.has(it.id)) {
            ...
            (0, contract_1.assertCatalogClean)(items);
            const tuDemPham = (0, validate_1.kiemTuDemVoiMucLuc)(pack, items);
            if (tuDemPham.length > 0) {
                ghi(`[muc-luc] ${tenant}: tu dem cua ho so "${pack.id}" nuot ten mon cua shop: ` +
                    `${tuDemPham.join(", ")} — sua ho so nganh, khong sua ten mon.`);
            }
            const mon = items.map((it) => {
        ...
        async search(tenant, query, limit) {
            ...
            return ml.mon
                .map((m, i) => ({ m, i, diem: (0, contract_1.coverage)(query, ml.chuoi[i] ?? "") }))
                .filter((x) => x.diem > 0)
...
//# sourceMappingURL=muc-luc.js.map