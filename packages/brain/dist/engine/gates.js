...
exports.scanNumbers = scanNumbers;
exports.numbersIn = numbersIn;
exports.wordQuantityClaims = wordQuantityClaims;
exports.runGates = runGates;
const memory_1 = require("./memory");
const text_1 = require("./text");
const SEVERITY = { send: 0, ask_back: 1, handoff: 2, block: 3 };
const RULE_PRIORITY = {
...
function scanNumbers(text) {
    let n = (0, text_1.normalize)(String(text ?? "").replace(TIEN_TE_RE, " "));
    const structured = [];
    const numbers = [];
    ...
    n = n.replace(COMPOUND_MONEY_RE, (_m, a, unit, b) => {
...
function wordQuantityClaims(text) {
    const n = (0, text_1.soft)(text);
    const out = [];
    for (const w of WORD_NUMBERS) {
        for (const c of CLASSIFIERS) {
            if (new RegExp(`(^|[^\\p{L}])${w}\\s+${c}([^\\p{L}]|$)`, "u").test(n))
                out.push(`${w} ${c}`);
        }
    }
    return out;
 ...
 */
function phraseHit(draft, phrase) {
    if ((0, text_1.soft)(draft).includes((0, text_1.soft)(phrase)))
        return true;
    // Dau cau chen vao giua khong duoc lam cum cam thoat: "rẻ nhất - thị trường".
    if ((0, text_1.squash)(draft).includes((0, text_1.squash)(phrase)))
        return true;
    const flat = (0, text_1.normalize)(phrase);
    if (flat.includes(" "))
        return (0, text_1.normalize)(draft).includes(flat);
    return false;
}
/**
 ...
 */
function patternHit(draft, pattern) {
    try {
        return new RegExp(pattern, "u").test((0, text_1.soft)(draft));
    }
    catch {
        return false;
    }
...
function evaluateRule(rule, input) {
    switch (rule.kind) {
        ...
        case "require_source_for_claims": {
            ...
            if (touched.length === 0 || input.hasPolicySource)
                return null;
            return {
                action: "block",
                rule: rule.kind,
        ...
        case "no_unsourced_numbers": {
            ...
            if (invented.length === 0 && inventedStructured.length === 0 && wordy.length === 0)
                return null;
            const parts = [];
            if (invented.length > 0)
                parts.push(`so khong truy duoc nguon: ${invented.join(", ")}`);
            if (inventedStructured.length > 0)
                parts.push(`gio/ngay khong co nguon: ${inventedStructured.join(", ")}`);
            if (wordy.length > 0)
                parts.push(`so luong viet bang chu: ${wordy.join(", ")}`);
            return { action: "block", rule: rule.kind, reason: `Cau tra loi co ${parts.join("; ")}.` };
        }
        case "brand_not_carried_needs_catalog": {
            if (!input.claimsBrandNotCarried)
                return null;
            if (input.catalogSize >= rule.minItems)
                return null;
            return {
                action: "ask_back",
                rule: rule.kind,
                reason: `Muc luc moi co ${input.catalogSize} mon (can it nhat ${rule.minItems}) — ` +
        ...
        case "forbidden_patterns": {
            for (const p of rule.patterns) {
                ...
                for (const t of input.toolText ?? []) {
                    try {
                        if (new RegExp((0, text_1.stripDiacritics)(p), "u").test((0, text_1.stripDiacritics)(t))) {
                            return {
...
function runGates(input) {
    ...
    let worst = { action: "send", reason: "Khong cong nao can." };
    for (const v of all) {
