...
import { assertNoStoredPII, dinhDangTien, redactPII, type CatalogItemLite, type ConversationId, type ItemId, type TenantId, type ToolInput, type ToolName } from "@sp/contract";
...
import { appendShopTurn, appendTurn, emptyState, hasRecentImageEvidence } from "./memory";
import { numbersIn, runGates, type Fact, type GateVerdict } from "./gates";
import { coverage, hasWord, mentionsBrand, normalize, specificTokens, tight, tokens } from "./text";
...
export function detectIntent(pack: IndustryPack, text: string): PackIntent | null {
  ...
  for (const intent of pack.intents) {
    ...
    for (const p of intent.patterns ?? []) {
      try { if (new RegExp(p).test(n)) score += 3; } catch { /* validatePack da bao */ }
    }
...
export function canonicalValue(axis: PackAxis, value: string): string {
  ...
  for (const rule of axis.canonical) {
    try { out = out.replace(new RegExp(rule.pattern), rule.replace); } catch { /* da bao */ }
  }
  return out.trim();
}
...
// ---------------------------------------------------------------- mau cau
interface Rendered { text: string; missing: string[] }

function render(template: string, vars: Record<string, string>, dataVars?: Set<string>): Rendered {
  const set = dataVars ?? new Set(BASE_DATA_VARS);
  const missing: string[] = [];
  const text = template.replace(/\{(\w+)\}/g, (_m, key: string) => {
    const v = vars[key];
    if (v === undefined || (v === "" && set.has(key))) missing.push(key);
    return v ?? "";
  });
  return { text: text.replace(/\s{2,}/g, " ").trim(), missing };
}
function tpl(pack: IndustryPack, key: string): string {
...
interface DispatchCtx {
  ...
  tenant: TenantId;
  conversationId: ConversationId;
  /** Moc gio cua luot nay — thanh phan thu hai cua khoa chong trung. */
  luotLuc: string;
  /** Cau khach go, da qua bang tu dong nghia va chuan hoa. */
  text: string;
  /** Truc nao co gia tri rut ra tu CHINH cau nay (khong phai dinh tu luot truoc). */
  slotsThisTurn: ReadonlySet<string>;
  /** Tuy chon cua `soTranTrongCau` cho luot nay: ten mon dang noi, so trong ten, dang tra loi. */
  soTuyChon: TuyChonSoTran;
  itemCode: string | null;
  itemId: ItemId | null;
  slots: Record<string, string>;
  state: ConversationState;
  vars: Record<string, string>;
  /** O mang du lieu cua ho so nay — cong cu phai canh giong nhu vong ngoai. */
  dataVars: Set<string>;
}
interface DispatchOut {
  facts: Fact[];
  /** O thong tin cong cu tu suy ra duoc tu cau khach (xem `doanTrucTuSoTran`). */
  slotsFound?: Record<string, string>;
  /**
   * Truc ma cong cu da GO ghim (khach neu mot con so moi cho truc do ma khong doan
   * duoc). Phai go ca o `slots` ngoai — khong thi luot sau gia tri cu quay lai, va cau
   * hoi ve 43 duoc tra loi bang 42 muon hon mot luot. Hai cho go ghim phai cung tuoi tho.
   */
  slotsGo?: string[];
  vars: Record<string, string>;
  answered: boolean;
  hasPolicySource?: boolean;
  /** Cong cu goi that bai — khac han "goi duoc nhung khong co du lieu". */
  failed?: boolean;
  /** Con thieu gia tri cua truc nay moi tra loi duoc. */
  needAxis?: string;
  /** Ket qua bi cat bot — khong duoc khang dinh gi tren no. */
  partial?: boolean;
  /** O thay the con rong trong cau ma cong cu tu dung. */
  missing?: string[];
}
 ...
 */
function goiCongCu<K extends ToolName>(
  c: DispatchCtx, tool: K, input: ToolInput<K>
...
/**
 * @param cueThem Tu trong TEN/MA MON DANG NOI. "paracetamol 650", "boston 43" — ten mon
 *   dung ngay truoc con so la dang pho bien nhat o ca hai nganh, va "mon vua noi + mot
 *   con so" la bang chung manh nhat rang con so do la bien the.
 */
/**
 * @param soTenMon Con so LA MOT PHAN TEN MON ma khach vua go trong chinh cau nay ("13"
 *   trong "adizero boston 13 nhe", "500" trong "co vitamin c 500 khong"). Khong bo qua thi
 *   "13" bi coi la mot so tran khong khop nhan nao, cong go ghim size 42 vua noi, va khach
 *   tra loi dung ten mon lai bi hoi size lan nua — roi chuyen nguoi that.
 *   CHI khi ten mon duoc go trong luot nay: "500 con khong shop" (khong go ten) thi "500"
 *   la cau tra loi ve ham luong, khong phai ten.
 */
interface TuyChonSoTran {
  /** Tu trong ten/ma mon dang noi — tinh la tu bao hieu, nhung van phai qua chot cau hoi. */
  cueThem?: ReadonlySet<string> | undefined;
  /** Con so la mot phan ten mon khach vua go — khong phai so tran. */
  soTenMon?: ReadonlySet<string> | undefined;
  /** Bot VUA HOI dung truc nay: con so mo dau tin la cau tra loi, tieu tu gi cung duoc. */
  dangTraLoi?: boolean | undefined;
}
...
function soTranTrongCau(
  pack: IndustryPack, text: string, tuyChon: TuyChonSoTran = {}
): number[] {
  const cueThem = tuyChon.cueThem ?? new Set<string>();
  const soTenMon = tuyChon.soTenMon ?? new Set<string>();
  ...
  for (const a of pack.itemShape.axes) {
    manh.add(normalize(a.id));
    for (const t of tokens(normalize(a.label))) manh.add(t);
  }
  const ra: number[] = [];
  ...
  for (const m of text.matchAll(/(?<!\d|\d[.,])(\d+(?:[.,]\d+)?)(?!\d|[.,]\d)/g)) {
    ...
    const gan = tokens(truoc).slice(-CUA_SO_DAT_HANG).join(" ");
 ...
 */
function doanTrucTuSoTran(
  pack: IndustryPack, axis: PackAxis, rows: readonly { variantLabel: string }[], text: string,
  tuyChon: TuyChonSoTran
): string | null {
  ...
  for (const x of rows) {
    ...
    if (Number(dau.replace(",", ".")) === so[0]) khop.add(v);
  }
  return khop.size === 1 ? ([...khop][0] ?? null) : null;
}

const DISPATCH: Partial<Record<ToolName, (c: DispatchCtx) => Promise<DispatchOut>>> = {
  "stock.lookup": async (c) => {
    if (c.itemCode === null) return { facts: [], vars: {}, answered: false };
    const r = await goiCongCu(c, "stock.lookup", { code: c.itemCode });
    if (!r.ok) return { facts: [], vars: {}, answered: false, failed: true };
    ...
    if (r.data.truncated) return { facts: [], vars: {}, answered: false, partial: true };
    ...
    const slots: Record<string, string> = { ...c.slots };
    let slotsFound: Record<string, string> | undefined;
    const slotsGo: string[] = [];
    for (const axis of axes) {
      if (!axis.requiredForStock || c.slotsThisTurn.has(axis.id)) continue;
    ...
    const vars: Record<string, string> = {
      ...
      gia: prices[0] === undefined ? "" : dinhDangTien(prices[0]),
      giacao: prices[prices.length - 1] === undefined
        ? "" : dinhDangTien(prices[prices.length - 1] as number),
  ...
  "policy.get": async (c) => {
    ...
    if (!r.ok) return { facts: [], vars: {}, answered: false, failed: true };
    if (!r.data.found) return { facts: [], vars: {}, answered: false };
    return {
  ...
  "order.lookup": async (c) => {
    ...
    if (phone === undefined) return { facts: [], vars: {}, answered: false };
    ...
    if (o === undefined) return { facts: [], vars: {}, answered: false };
    return {
      facts: [{
        source: "order.lookup",
        ...
        numbers: [
          ...
          ...numbersIn(String(o.orderId)), ...numbersIn(o.status)
        ]
      }],
      vars: {
        madon: String(o.orderId), trangthai: o.status,
        conphaitra: dinhDangTien(o.money.remaining)
      },
      answered: true
    };
  },
  ...
  "purchase.eta": async (c) => {
    if (c.itemId === null) return { facts: [], vars: {}, answered: false };
...
export const DISPATCHABLE_TOOLS = Object.keys(DISPATCH) as ToolName[];
...
export async function handleTurn(
  ...
  const base = loaded ?? emptyState(input.tenant, input.conversationId);
  let state = appendTurn(base, { role: "customer", text: input.text, at, imageCount: input.imageCount }, now);

  const online = ports.tools.online();
  const available = new Set<ToolName>(ports.tools.available());
  ...
  const coSoHaiChuSo = tokens(normText).some((t) => /^\d{2,}$/.test(t));
  if (specific.length > 0 || coSoHaiChuSo) {
    ...
    if (top !== undefined && topScore >= ITEM_MATCH_THRESHOLD && topScore > secondScore) {
      ...
      tuTenMon = new Set(tokens(`${top.code} ${top.name}`));
      ...
      const coMonKhac = !traLoiBangSo && top !== undefined && top.code !== state.focusItemCode
        && topScore >= ITEM_MATCH_THRESHOLD * 0.6 && khopChu;
      ...
      const soLaTenMon = !traLoiBangSo && tokens(normText).some((t) =>
        /^\d{2,}$/.test(t)
        && !axes.some((a) => extractAxis(a, t) !== null)
        && candidates.some((it) =>
          it.code !== state.focusItemCode && tokens(`${it.code} ${it.name}`).includes(t)));
      if (!stillAboutFocus && (coMonKhac || coTuChuLa || soLaTenMon)) { itemCode = null; itemId = null; }
    }
  }
  ...
  const slots: Record<string, string> = itemChanged ? {} : { ...(state.focusSlots ?? {}) };
  const echoed: string[] = [];
  /** Truc nao co gia tri rut ra tu CHINH cau nay — khac voi gia tri dinh tu luot truoc. */
  const slotsThisTurn = new Set<string>();
  ...
  const cueThem = new Set<string>([
    ...(dau !== undefined && (itemNamedThisTurn || dau.code === state.focusItemCode)
      ? tokens(`${dau.code} ${dau.name}`) : []),
    ...(state.focusItemCode === undefined ? [] : tokens(state.focusItemCode))
  ]);
  const soTenMon = new Set<string>(
    itemNamedThisTurn ? [...tuTenMon].filter((t) => /^\d/.test(t)) : []
  );
  const soTuyChon: TuyChonSoTran = {
    cueThem, soTenMon,
   ...
   */
  const slotsDoan: Record<string, string> = {};
  for (const axis of axes) {
    ...
    slots[axis.id] = v; echoed.push(v); slotsThisTurn.add(axis.id);
  }
  const phone = /(?:^|\D)(0\d{9})(?:\D|$)/.exec(input.text.replace(/[.\s-]/g, ""))?.[1];
  if (phone !== undefined) { slots["phone"] = phone; echoed.push(phone); }
  ...
  let needAxis: string | undefined;
  const toolMissing: string[] = [];

  if (intent !== null && online) {
    for (const tool of intent.tools) {
      if (!available.has(tool) || !pack.allowedTools.includes(tool)) continue;
      ...
      if (out.slotsFound !== undefined) {
        for (const [k, v] of Object.entries(out.slotsFound)) { slotsDoan[k] = v; echoed.push(v); }
      }
      for (const k of out.slotsGo ?? []) delete slots[k];
      toolText.push(...out.facts.map((x) => x.text));
      Object.assign(vars, out.vars);
      if (out.answered) answered = true;
      if (out.hasPolicySource === true) hasPolicySource = true;
      if (out.failed === true || out.partial === true) toolFailed = true;
      if (out.needAxis !== undefined) needAxis = out.needAxis;
      if (out.missing !== undefined) toolMissing.push(...out.missing);
    }
  }
  ...
  if (needAxis !== undefined && !missing.includes(needAxis)) missing.push(needAxis);
  ...
  if (state.handedOff === true || intent?.handoff === true || brokenTemplate ||
    ...
    if (askedSlot === "item" || askedSlot === undefined) {
      ...
      const key = hasRecentImageEvidence(state, now) && tpl(pack, "ask_item_has_image") !== ""
        ? "ask_item_has_image"
        : "ask_item";
  ...
  const gate2 = reply === draft.text
    ? { verdict: { action: "send" as const, reason: "" }, all: [] as GateVerdict[] }
    : runGates({
  ...
  if (cauDuPhongHong || replyMissing.length > 0) {
    ...
    reply = cuoi.missing.length > 0 || runGates({
  ...
  assertNoStoredPII([...state.turns.map((t) => t.text), ...Object.values(slotsLuu)]);
