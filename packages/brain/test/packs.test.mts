...
import { ask, clonePack, fakePorts, PARA, pharmacyPack, runningShoesPack } from "./fixtures.mts";

test("both built-in packs are valid", () => {
  assert.doesNotThrow(() => B.selfCheckPacks());
  assert.deepEqual(B.validatePack(runningShoesPack), []);
  assert.deepEqual(B.validatePack(pharmacyPack), []);
});
...
test("PROMISE: two axes are read from one sentence", () => {
  const axes = pharmacyPack.itemShape.axes;
...
test("the validator catches broken packs", () => {
  const a = clonePack(runningShoesPack); a.templates["handoff"] = "Nhan vien goi lai trong 5 phut nhe";
  assert.ok(B.validatePack(a).some((m) => /khong duoc chua con so/.test(m)));

  const b = clonePack(runningShoesPack); b.intents[0]!.tools = ["shipment.track"];
  assert.ok(B.validatePack(b).some((m) => /khong nam trong danh sach cho phep/.test(m)));

  const c = clonePack(runningShoesPack); c.gates = c.gates.filter((g) => g.kind !== "no_unsourced_numbers");
  assert.ok(B.validatePack(c).some((m) => /thieu cong bat buoc/.test(m)));

  const d = clonePack(runningShoesPack); d.intents[0]!.requiredSlots = ["khung_gio"];
  assert.ok(B.validatePack(d).some((m) => /khong ton tai/.test(m)));

  const e = clonePack(runningShoesPack); e.templates["in_stock"] = "Con {ton} doi {khunggio} a";
...
test("the validator inspects templates INSIDE intents too", () => {
  ...
  const a = clonePack(pharmacyPack);
  ...
  const b = clonePack(pharmacyPack);
  ...
  const c = clonePack(pharmacyPack);
  ...
  const d = clonePack(pharmacyPack);
...
test("pack-specific placeholders must come with a value", () => {
  const good = clonePack(runningShoesPack);
...
test("filler words live in the PACK, not in the engine", () => {
  ...
  assert.ok(pharmacyPack.lexicon.fillerWords !== undefined);
  assert.ok(!pharmacyPack.lexicon.fillerWords.includes("cam"), "'cam' is swallowed at the pharmacy.");
});

test("a pack's filler words must not swallow a merchant's item names", () => {
  assert.deepEqual(B.checkFillerWordsAgainstCatalog(runningShoesPack, [{ code: "GM1", name: "Giày mọi da bò" }]), []);
  const bad = clonePack(runningShoesPack);
  ...
  assert.deepEqual(B.checkFillerWordsAgainstCatalog(bad, [{ code: "GM1", name: "Giày mọi da bò" }]), ["moi"]);
  assert.ok(B.specificTokens("giay moi con size 42 khong", runningShoesPack.lexicon.genericTerms,
    runningShoesPack.lexicon.fillerWords).includes("moi"), "'giay moi' could never be sold.");
});
...
test("'doi' as the counter for shoes is not read as an exchange request", () => {
  assert.notEqual(B.detectIntent(runningShoesPack, "doi nay dep qua shop")?.id, "doi_tra");
  assert.equal(B.detectIntent(runningShoesPack, "shop cho doi size khong a")?.id, "doi_tra");
});

test("quantities and other units are not read as sizes", () => {
  const axis = runningShoesPack.itemShape.axes[0]!;
  for (const text of [
...
test("misspelled brands are still recognised through aliases", () => {
  assert.match(B.applyAliases(runningShoesPack, "co giay niubalan khong"), /new balance/);
  assert.match(B.applyAliases(runningShoesPack, "mau bostom 13"), /boston/);
});

test("warehouse labels written differently still match the customer's value", () => {
  const axis = pharmacyPack.itemShape.axes[0]!;
