...
exports.findPIIInText = findPIIInText;
exports.assertNoStoredPII = assertNoStoredPII;
exports.REDACTED = "[da che]";
/**
 ...
 */
const REDACT_PHONE_RE = /(?<![\p{L}\d])(?:\+?84|00?84|[0oO])[\s._,|*x()\-]{0,3}(?:3|5|7|8|9|1[2689]|2)(?:[\s._,|*x()\-]{0,3}\d){7,10}(?!\d)/gu;
/**
 ...
 */
const DETECT_PHONE_RE = /(?<![\p{L}\d])(?:\+?84|0)[\s._,|*x()\-]{0,3}(?:3|5|7|8|9|1[2689]|2)(?:[\s._,|*x()\-]{0,3}\d){7,9}(?![\d])/u;
...
function redactPII(text) {
    return String(text ?? "")
        .replace(new RegExp(EMAIL_RE.source, "gi"), exports.REDACTED)
        .replace(REDACT_PHONE_RE, exports.REDACTED);
}
...
function findPIIInText(text) {
    ...
    if (email !== null)
        out.push({ index: email.index, kind: "email" });
    const phone = new RegExp(DETECT_PHONE_RE.source, "u").exec(s);
    if (phone !== null)
        out.push({ index: phone.index, kind: "phone" });
    return out;
}
/**
 ...
 */
function assertNoStoredPII(texts) {
    const bad = [];
    texts.forEach((t, i) => {
        for (const f of findPIIInText(t))
            bad.push(`[${i}] ${f.kind}`);
    });
    if (bad.length > 0) {
        throw new Error(`Van ban sap luu con du lieu ca nhan (${bad.length} cho): ${bad.slice(0, 6).join(", ")}. ` +
